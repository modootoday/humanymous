	...
	for cx = 20; cx <= 80; cx += 4 {
		for cy = 25; cy <= 80; cy += 4 {
			for ang = -1.1; ang <= 1.1; ang += 0.2 {
				if Verify(testKey, sid, bucket, bucket, cx, cy, ang) {
...
func TestGenerateDeterministic(t *testing.T) {
	a := Generate(testKey, "sess-1", 100)
	b := Generate(testKey, "sess-1", 100)
	if !reflect.DeepEqual(a, b) {
		t.Fatal("Generate must be deterministic for the same seed")
	}
	c := Generate(testKey, "sess-2", 100)
...
func TestSceneIsSolvable(t *testing.T) {
	// Every generated instance must admit at least one passing ramp placement.
	for _, sid := range []string{"s-a", "s-b", "s-c", "s-d", "s-e"} {
		cx, cy, ang, ok := findSolution(t, sid, 100)
		if !ok {
			sc := Generate(testKey, sid, 100)
			t.Errorf("no solution found for %s (ball=%v cup=%v r=%.1f) — simulator or generator broken", sid, sc.Ball, sc.Cup, sc.CupR)
			continue
		}
		if !Verify(testKey, sid, 100, 100, cx, cy, ang) {
			t.Errorf("found solution does not re-verify for %s", sid)
		}
	}
}
...
func TestStaleBucketRejected(t *testing.T) {
	...
	if Verify(testKey, "s-a", 100, 100, cx, cy, ang) != true {
		t.Fatal("sanity: fresh solution should verify")
	}
	if Verify(testKey, "s-a", 100, 104, cx, cy, ang) {
		t.Error("a solution submitted 4 buckets late must be rejected (TTL)")
	}
	if Verify(testKey, "s-a", 100, 99, cx, cy, ang) {
