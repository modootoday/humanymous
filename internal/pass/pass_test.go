...
func TestGenerateDeterministic(t *testing.T) {
	...
	if c := Generate(testKey, "sess-2", 100, 1); reflect.DeepEqual(a, c) {
...
func TestSolutionVerifies(t *testing.T) {
	for d := 0; d <= 3; d++ {
		for _, sid := range []string{"a", "b", "c", "d"} {
			sol := SolutionOffsets(testKey, sid, 100, d)
			if !Verify(testKey, sid, 100, 100, d, sol) {
...
func TestWrongOffsetsFail(t *testing.T) {
	sol := SolutionOffsets(testKey, "a", 100, 1)
	bad := append([]int(nil), sol...)
	bad[0] = (bad[0] + 1) % 11 // nudge one row off centre
	if Verify(testKey, "a", 100, 100, 1, bad) {
		t.Error("a misaligned row must fail")
	}
	if Verify(testKey, "a", 100, 100, 1, []int{0, 0}) {
...
func TestStaleBucketRejected(t *testing.T) {
	sol := SolutionOffsets(testKey, "a", 100, 1)
	if !Verify(testKey, "a", 100, 100, 1, sol) {
		t.Fatal("sanity: fresh solution should verify")
	}
	if Verify(testKey, "a", 100, 104, 1, sol) {
		t.Error("a solution 4 buckets late must be rejected (TTL)")
	}
	if Verify(testKey, "a", 100, 99, 1, sol) {
