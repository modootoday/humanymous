# Quickstart: Put Sentinel in front of an app in monitor mode (first 30 minutes)

**Tutorial** (learning-oriented) · **Audience:** integrator or evaluator, first run.

Welcome. In the next 30 minutes you will stand up **humanymous Sentinel** — the reverse-proxy enforcement layer — in front of a throwaway app on your own machine, running in **global monitor mode**. Monitor mode is where every deployment should start: Sentinel scores and logs every request across layers L1–L7, but it enforces *nothing*. No visitor is challenged, no visitor is blocked, and your origin app behaves exactly as it did before. You get to watch the detection work with zero risk to real traffic.
...
- **Go** installed (to build the binary from `./cmd/sentinel`).
...
- **9000** — your throwaway origin app (what Sentinel fronts).
- **8444** — Sentinel's public edge (HTTPS).
- **8445** — Sentinel's separate admin listener (the Audit Console).

## Step 1 — Start a throwaway origin on 127.0.0.1:9000

Sentinel needs an origin to sit in front of. Any app that returns HTML will do. For this tutorial, serve a tiny static page.
...
> **Tip:** This is a plain, unprotected origin — exactly the app you are about to place Sentinel in front of. In a real deployment, Sentinel terminates TLS at the edge and forwards to your origin over the private network; the origin itself is not exposed to the internet.

## Step 2 — Build the binary

From the repository root, build the Sentinel binary:

```
go build -o bin/sentinel.exe ./cmd/sentinel
```

When it finishes, the command returns with no output and `bin/sentinel.exe` exists. That is success — `go build` is quiet when it works.

## Step 3 — Run Sentinel in global monitor mode

Now start Sentinel. The `-monitor` flag is the important one: it puts the whole fleet in **global monitor mode**, which downgrades every route to monitor. Sentinel will score and log, but enforce nothing, everywhere.

```
bin/sentinel.exe -addr :8444 -upstream http://127.0.0.1:9000 -monitor
```

You should see startup log lines like these:

```
humanymous Sentinel on https://localhost:8444 -> http://127.0.0.1:9000 (monitor=true)
...
Leave Sentinel running.

## Step 4 — Load your app through Sentinel
...
You should now see your origin's page — the "Hello from my origin" heading you created in Step 1. Sentinel fronted the request, terminated TLS, forwarded to `127.0.0.1:9000`, and streamed the response back.

Here is the interesting part: the HTML that came back is *not* byte-for-byte your origin's HTML. Sentinel injected the detection bundle into it. To confirm this, view the page source (or fetch it directly) and look for a reference to the control path `/__hmn/` — the injected bundle loads via the control plane, for example `/__hmn/loader.js`.
...
Look in the returned HTML for your original `<h1>Hello from my origin</h1>` **and** the injected reference to `/__hmn/`. Seeing both is the proof: your app's content is intact, and Sentinel's detection bundle is now riding along with it.

The exact markup Sentinel inserts once at the first `<head>` boundary is:

```
<!--hmn-injected--><script src="/__hmn/loader.js" defer></script>
```

The `<!--hmn-injected-->` comment is an idempotency marker — if it is already present, Sentinel passes the page through untouched, so injection never happens twice.

## Step 5 — Open the Audit Console and watch a verdict appear
...
Go to the **Overview** view ("live edge decisions"). As you reload `https://localhost:8444` in your other browser tab, watch a verdict flow in for each request. Every request Sentinel handled — including your own page loads — was scored across layers L1–L7 and written to the tamper-evident audit log.
...
The verdict on the stream is a *decision that was recorded, not enforced*. That is exactly the point of monitor mode: you get full visibility into what Sentinel would decide, with no effect on live traffic.

## You did it

In about half an hour you have:

- Stood up an origin, built Sentinel, and placed it at the edge in front of your app.
