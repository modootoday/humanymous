...
> **Confirmed in source:** Tier transitions are not automated by a background job. The reference models retention as a pure classifier — `RetentionPolicy.Tier(age)` maps a record's age to `HOT`/`WARM`/`COLD`/`EXPIRED` on demand — plus the default windows (HOT 90d, WARM 1y, COLD 7y). No scheduler migrates records between tiers, and physical retirement of expired segments (moving checkpoint-bounded prefixes to WORM and recording their terminal STH) is the production step, not shipped in the reference. Tiering is a queryable policy the reference computes; acting on it is operator-driven (prod-delta).
