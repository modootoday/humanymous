...
keywords: ["GDPR Article 30 RoPA","data processing inventory","bot detection GDPR","pseudonymization scrypt 64-hex","cryptographic erasure crypto-shred","Article 17 right to erasure","legitimate interest bot mitigation","IP JA3/JA4 pseudonym","data protection officer record of processing activities","humanymous Gate"]
---
...
Publish the concrete retention periods, your identity as controller, and your data protection officer/contact
alongside this notice.

> **If you enable the behavioral training-trace store (`-ml-trace-dir`, §2):** the "used **only** for
> security" line above is no longer complete — confirmed-human interaction aggregates are then retained
> for **model retraining**, which is arguably a distinct purpose. Amend the notice to add model
> improvement/retraining as a purpose (or record it as a compatible secondary use of the security basis)
> and reflect its separate retention, so you are not under-notifying under Article 13(1)(c).
...
> - **Cross-session correlation registry.** An **in-memory, TTL-bounded** graph of `fingerprint → {/24 subnets, session ids}` used to detect residential-proxy rotation. In single-node (default) mode it is per-process, never persisted, and swept on its TTL — *transient in-memory processing, not written to disk*. **When shared-fleet mode (`-redis`) is enabled on the Core**, the per-fingerprint distinct-`/24` set and a subnet-arrival window are additionally written to the Redis coordinator (keys `hmn:corr:sub:<h>` / `hmn:corr:vel:<h>`), so a campaign split across nodes is counted as one. The **key** is an HMAC/SHA-256 digest of `fingerprintId|ja4` (no raw fingerprint bytes at rest), but the **values are the raw `/24` subnet prefixes + arrival timestamps** — a device-fingerprint → {network locations, timing} graph, i.e. personal data held in the coordinator. It is **bounded by a ~60-minute key TTL** (its saving grace: self-clearing) and is **outside the Article 17 crypto-shred boundary** — the digest is not the per-subject linkage key, so a subject-erasure cannot reach it; rely on the TTL and disclose it as a coordinator-side store. **Set `HMN_REDIS_KEY`** in any `-redis` deployment so the key is HMAC-keyed rather than a bare SHA-256 hash (an unkeyed digest is a reversible confirmation oracle for enumerable fingerprints). *Add this store to your RoPA when `-redis` is enabled; single-node mode adds nothing here.*
> - **Operational diagnostics.** Core and Gate can append formatted plain text and JSON Lines to an explicitly configured logging store. The reference call sites prohibit raw or shortened IP addresses, URLs, headers, cookies, bodies, session identifiers, administrator tokens, panic values, and stacks. These best-effort diagnostics are separate from the audit log, are not covered by audit crypto-shred, and require an operator-defined short retention and access policy. Keep the logging store on a different failure domain from audit and key storage.
> - **Behavioral training-trace store (Core, off by default, lab/operator opt-in).** When the Core is run with a trace directory configured (see [Operate the behavioral model](../how-to/operate-behavioral-model.md)), it appends one JSON Lines record per **solved challenge** — a confirmed human — for offline model retraining. **This is a second at-rest copy of the row-8 behavioral data that is aggregated but *not* pseudonymized and *not* in the audit chain**, so it must be inventoried on its own. Each record holds a label, the session's **aggregate** behavioral summary (the same timings-only statistics as row 8 — no key values, no raw event stream), a unix timestamp, an input-modality cohort, and a source tag; the file is written `0600` with no scrypt pseudonym, no encryption at rest, and no hash chain / HMAC / witness — its only protection is filesystem permissions. Treat it as **pseudonymous, not anonymous**: it carries no session id, but the timestamp plus the exact behavioral summary are a join key back to the timestamped audit decision for the same session. The cohort value (`keyboard` / `pointer` / `default`) is a **coarse input-modality inference** that correlates with assistive-technology use; it is not a declared disability or health record, but whether an inferred AT-adjacency attribute is special-category data under Article 9 for your deployment is a **controller determination** — record it. The store is **append-only with no TTL and no rotation** (operator-defined retention, kept on a separate failure domain), and it is **outside the crypto-shred erasure boundary**: with no per-subject linkage key, a subject-erasure request cannot locate that subject's trace, so add the file to your Article 17 procedure with a **file-level retire policy**. Finally, offline retraining is arguably a **distinct purpose** from live security scoring — decide and record whether it is a compatible secondary use of your legitimate-interest basis or needs its own basis and notice.
...
Detection-layer references (seven detection stages) are defined in [How Gate sees a request](../concepts/how-gate-sees-a-request.md). "Observed form" is what Gate reads during scoring; "Stored form" is what is persisted to the audit log.

| # | Identifier / signal | Detection layer | Purpose of processing | Observed form (transient) | Stored form (at rest) |
|---|---------------------|-----------------|-----------------------|---------------------------|-----------------------|
| 1 | IP address | network and protocol inspection stage (network/protocol) | Automated-traffic detection; rate metering; ban keying | Raw source address, in memory | 64-hex pseudonym only |
| 2 | JA3 / JA4 TLS fingerprint | network and protocol inspection stage | TLS-stack consistency vs. claimed browser | Raw fingerprint, in memory | 64-hex pseudonym only |
| 3 | HTTP/2 fingerprint | network and protocol inspection stage | Protocol-stack consistency; HTTP/2 abuse detection | Raw fingerprint, in memory | 64-hex pseudonym only |
| 4 | User-Agent (UA) | static client inspection stage / consistency-check stage | Claimed-client baseline; cross-check vs. TLS/JS | Raw header, in memory | 64-hex pseudonym only |
| 5 | User-Agent Client Hints (UA-CH) | consistency-check stage | Cross-check of UA against UA-CH and JS evidence | Raw hints, in memory | 64-hex pseudonym only |
| 6 | SNI (server name indication) | network and protocol inspection stage | TLS handshake context | Raw SNI, in memory | 64-hex pseudonym only |
| 7 | Device fingerprint (canvas/WebGL/audio hash, screen, hardware) | client fingerprinting stage | Client fingerprint; cross-session correlation of automated traffic | Derived fingerprint, in memory | 64-hex pseudonym only |
| 8 | Behavioral signals (mouse, keystroke, scroll dynamics; `isTrusted`) | interaction analysis stage | Human-interaction evidence; automation cadence detection | Event-derived features, in memory | Pseudonymized / aggregated in the record |
...
| 10 | Verdict, risk score, contributing signal ids, route/host | scoring and policy stage / edge | The decision record itself (audit evidence) | Computed at the edge | Recorded in the audit log |
| 11 | Resource-watermark ledger (session id + **keyed** IP pseudonym) | network and protocol inspection stage (resource/watermark) | Per-session forensic leak-tracing of served resources | Session id + HMAC(masterKey, IP) token, in memory | **In-memory only, ~24h TTL; self-expires — NOT persisted to the audit log and OUT of the crypto-shred erasure scope (it is not resolvable after TTL).** |
...
> reference build applies no Article 9 special-category handling by default. If you enable the
> optional behavioral training-trace store (section 2), these same aggregates are additionally
> written to disk un-pseudonymized — inventory that store separately.

> **Note (low significance).** Gate sets three client-side cookies/tokens as part of enforcement — the verdict trust token (`hmn_vt`), the rotating request-integrity token (request-integrity token), and the attestation step-up proof (`hmn_su`, minted on an attested route after a Pass solve). These are **session-scoped, short-TTL, and not persisted** to the audit log; each expires on its own and is discarded. They carry no new identifier: they bind only to data already inventoried above — the IP/subnet (row 1), the device fingerprint (row 7), and the session id (row 9). They therefore add **no new personal-data category** to this inventory. Record them, if at all, as transient enforcement state derived from existing rows.

---

## 4. Pseudonymization policy note

This section is the pseudonymization policy statement a data protection officer can cite directly.
...
> **Confirmed in source:** For each subject — keyed by the session id — the vault mints a random 32-byte linkage key (`K_subj`) on first use and holds it in the vault; the session id is the handle to that key, not the key material itself. Each field's pseudonym is `scrypt(K_subj, salt = raw_field_value, N = 2¹², r = 8, p = 1, dkLen = 32)`, rendered as a 64-hex string (when stretching is disabled the reference falls back to `HMAC-SHA256(K_subj, raw_field_value)`, the same 64-hex shape). The raw field value doubles as the scrypt salt, which provides per-field domain separation: two different identifiers under the same subject produce unlinkable pseudonyms, while the same identifier under the same subject is deterministic — so a subject's own events stay linkable. There is no separate global pepper and no additional per-field salt; destroying `K_subj` (crypto-shred) severs all of that subject's pseudonyms at once.
...
> **Confirmed in source:** In the reference the only record-level tag field is `data_class` (JSON `data_class`, `omitempty`) on the audit record; there is no per-record `legal_basis` field. The reference defines no enumerated value set for `data_class` and never populates it (carried for forward schema compatibility, empty in emitted records), so any data-class vocabulary is an operator/production responsibility you define. Legal basis is captured elsewhere: it is a required free-text field on the crypto-shred erasure path (`LegalBasis`, e.g. `"GDPR Art.17"` or `"PIPA"`), recorded on the erasure request/decision and certificate — not as a tag on ordinary traffic records.
...
> **Honest residual:** Without `-audit-wal` the chain is **RAM-only and lost on restart**. With `-audit-wal` the write-ahead log **grows without bound**. Neither path implements a retention schedule. Set calendar retention, archival, and delete/retire policy yourself (production responsibility). The Ledger Retention card is hardcoded labels, not live policy.

> **Confirmed in source:** `RetentionPolicy.Tier(age)` has no non-test production caller that prunes or relocates storage. Physical WORM retirement is not shipped.
...
Crypto-shred is data protection officer-gated and dual-control, with a cancellable hold window before the shred commits. The full procedure, role gates, and hold-window handling are in the [Right-to-erasure (crypto-shred) runbook](../runbooks/erasure-crypto-shred.md).
...
> **Confirmed in source:** The reference produces no on-disk copy of the vault beyond the single sealed keystore file. The vault's per-subject linkage keys are serialized (`Vault.Snapshot`) into the `Vault` field of `KeyMaterial` and sealed — together with the signing and HMAC keys — into one file written by `SealKeys` (scrypt-derived AES-256-GCM, mode 0600). No separate vault file is written anywhere, and the durable audit sinks (write-ahead log, Redis, ClickHouse) persist only pseudonymized records, never linkage-key material. Your backups of that one keystore file are the entire on-disk vault surface to inventory.

---

## Related pages

- [How Gate sees a request](../concepts/how-gate-sees-a-request.md) — the pseudonymization model, Core stages, and Gate collection boundary.
- [Right-to-erasure (crypto-shred) runbook](../runbooks/erasure-crypto-shred.md) — the Article 17 procedure.
- [Compliance & data protection officer start here](../start-here/compliance-dpo.md) — orientation for the data protection officer role.
- [role-based access control and separation of duties](rbac-separation-of-duties.md) — the dual-control model that gates erasure, permanent/CIDR bans, and the kill switch (not re-identification).
