...
**Reference** (canonical policy) · **Audience:** Red-team developers and evaluators, read this **before** running or extending any bundled attack tooling.
...
This tooling exercises **your own detector's test catalog on your own loopback** — the standalone detection engine on `127.0.0.1:8443`. It is a way to confirm that your detector fires the verdicts and hard rules you expect against known-automated samples, and to measure your own honest-human false-positive picture before you enforce anything.
...
- **The T4 boundary is not solved.** Anti-detect tooling combined with real-human click-farms is a design boundary that is **mitigated by rate and reputation, not solved**. A pass or a block against a T4-style sample does not close that gap.
- **HR-11, HR-12, and HR-17 are heuristics that can challenge real humans.** HR-11 (a consistent browser from a datacenter ASN), HR-12 (zero interaction over the observation window), and HR-17 (a replayed or absent RIT token on an API call) issue **CHALLENGE**, not DENY, and can catch some genuine people. Do not report them as clean bot-only signals.
- **Every number is reference-measured on your machine.** A detection rate or false-positive rate describes **that run on your box** — it is not a promise about live traffic. Inspect the honest-human baseline's challenge-rate separately; a human CHALLENGE is scored as a true negative, so a DENY-only false-positive rate under-reports human friction.

For the verdict thresholds and the full HR-1..HR-21 table, see **[Hard rules, verdicts and signal-ID reference](hard-rules-verdicts.md)**.
...
> optimizing them into an evasion. Honest ceilings apply: the T4 boundary
> (anti-detect tooling plus real-human click-farms) is rate/reputation-mitigated,
> not solved; HR-11 / HR-12 / HR-17 are heuristics that can challenge real
...
- **[Red-team catalog](red-team-catalog.md)** — the profiles, their expected hard rules, and the real `cmd/redteam` attack values.
...
- **[Hard rules, verdicts and signal-ID reference](hard-rules-verdicts.md)** — verdict thresholds and the HR-1..HR-21 table.
