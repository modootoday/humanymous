...
| Cryptographic erasure (crypto-shred): destroy the per-subject linkage key; the chain and Merkle anchors stay intact and verifiable; a signed erasure certificate is sealed on commit | GDPR Art. 17 (right to erasure) | A repeatable, verifiable erasure mechanism that preserves audit integrity while severing re-identifiability. | [Erasure and crypto-shred runbook](../runbooks/erasure-crypto-shred.md) |
| Records-of-Processing extract exportable for review | GDPR Art. 30 (records of processing activities) | Provides the processing-activity content you assemble into your RoPA. | [Data processing inventory](./data-processing-inventory.md) |
...
The certificate is signed under the Ed25519 STH key (RFC 8032), so a recipient can verify it with the public key alone. The chain and Merkle anchors remain intact after a shred, which the Integrity view confirms independently (see step 4).
