...
|---|---|---|---|
| Ed25519 signing of the Signed Tree Head (STH) | RFC 8032 (EdDSA / Ed25519) | Signed, publicly verifiable checkpoints over the audit hash chain; verification "with the public key alone." | Integrity view / `GET /__hmn/admin/integrity`; [Verify the audit log](../how-to/verify-audit-log.md) |
...
| Control | Standard / requirement | What it supports | Where to verify |
|---|---|---|---|
| Raw identifiers (IP, JA4, HTTP/2 fingerprint, UA, SNI, device fingerprint) stored **only** as per-subject-key-derived pseudonyms (64-hex, scrypt-stretched) | GDPR Art. 4(5) + Recital 26 (pseudonymization) | Pseudonymization by default; identifiers are pseudonymous, **not anonymous** — re-identification needs the vault plus dual-control. | [Data processing inventory](./data-processing-inventory.md) |
| Data-protection-by-design defaults: pseudonymize-at-write, fail-closed edge on strict/unsafe requests, least-privilege RBAC | GDPR Art. 25 (data protection by design and by default) | Controls that support privacy-preserving defaults rather than opt-in configuration. | [RBAC and separation of duties](./rbac-separation-of-duties.md) |
...
| Cryptographic erasure (crypto-shred): destroy the per-subject linkage key; the chain and Merkle anchors stay intact and verifiable; a signed erasure certificate is sealed on **execution**, after the hold window elapses (a cancelled erasure produces no certificate) | GDPR Art. 17 (right to erasure) | A repeatable, verifiable erasure mechanism that preserves audit integrity while severing re-identifiability. | [Erasure and crypto-shred runbook](../runbooks/erasure-crypto-shred.md) |
...
| Control | Standard / requirement | What it supports | Where to verify |
|---|---|---|---|
| Crypto-shred at end of retention hold; signed erasure certificate; retention-tier classifier (HOT / WARM / COLD) | PIPA destruction obligations | Verifiable destruction of the linkage that makes records re-identifiable, with a certificate as destruction evidence. | [Erasure and crypto-shred runbook](../runbooks/erasure-crypto-shred.md) |
...
The following are established anchors a production deployment would add. They are **prod-delta**: they are **not implemented** in the reference build. Do not represent them as shipped features in a vendor assessment.

| Recommended anchor | Standard | Status |
|---|---|---|
| Trusted timestamps on STH checkpoints | RFC 3161 (Time-Stamp Protocol) | **Prod-delta — not shipped.** Recommended production anchor for externally attestable checkpoint time. |
| External WORM / immutable object storage for chain and checkpoints | S3 Object-Lock / WORM retention | **Prod-delta — not shipped.** Recommended production anchor for off-node immutability. |
...
- [ ] **RBAC and dual-control configuration** — separation of duties (Art. 25 / Art. 32)
...
There is **no admin endpoint that returns the certificate** (prod-delta). The `/__hmn/admin/erasures` endpoint lists only erasures still **within** their hold window (scheduled but not yet executed); once the shred runs, the certificate is observable only as an `erasure.completed` **audit record** in the stream:
...
The report verifies the hash chain, per-record HMAC, and Ed25519-signed checkpoints, and reports any mismatch by class: `hash-break`, `hmac-invalid`, `seq-gap`, `linkage-break`, `checkpoint-mismatch`, or `node-missing`. An assessor can re-verify with the public key alone. See [Verify the audit log](../how-to/verify-audit-log.md) for the full verification walkthrough and what each mismatch class means.
...
> **Note:** Observability in the reference build is the audit stream, the Integrity view/endpoint, the Overview KPIs, Prometheus `GET /__hmn/admin/metrics` on the admin plane, and edge `GET /__hmn/healthz` / `GET /__hmn/readyz`. SIEM shipping of the audit stream is still a prod-delta.

### 6. RBAC and dual-control configuration
...
Pair this with [RBAC and separation of duties](./rbac-separation-of-duties.md), which documents the four roles and their capabilities:

- **Auditor** — read only.
- **Operator** — read + operate (temp bans, lifts).
- **Approver** — read + approve.
- **DPO** — read + operate + approve, and the **only** role that can approve erasure.

Dual-control facts to state in the pack: a permanent or CIDR ban and the kill switch must be committed by a **distinct Approver**; an **erasure must be committed by a distinct DPO** (a generic Approver cannot). Temporary bans and lifts are single-Operator.
...
- [RBAC and separation of duties](./rbac-separation-of-duties.md) — roles, capabilities, dual-control
- [Erasure and crypto-shred runbook](../runbooks/erasure-crypto-shred.md) — right-to-erasure procedure
- [Hard rules and verdicts](./hard-rules-verdicts.md) — verdict and hard-rule glossary
- [Production vs reference](./production-vs-reference.md) — the full prod-delta boundary
