...
Release notes are generated by **git-cliff** (`cliff.toml`) from Conventional Commits, tag-driven on `v*` SemVer tags, and wired into `.github/workflows/release.yml` for release bodies. They are published on the [GitHub Releases page](https://github.com/modootoday/humanymous/releases) in [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) grouping — there is no committed `CHANGELOG.md` file in the repo.
