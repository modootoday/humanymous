...
Sentinel is written in pure Go with no CGO. That keeps the build simple and lets you cross-compile for any target with `GOOS`/`GOARCH` — you do not need a C compiler or platform-specific build tooling.

The Go module is `github.com/modootoday/humanymous`. Its only direct dependency is uTLS, which Sentinel uses to read TLS ClientHello fingerprints (part of the L5 network layer — see [how Sentinel sees a request](../concepts/how-sentinel-sees-a-request.md)).

## Supported platforms

Because the build is pure Go, Sentinel builds and runs on Linux, macOS, and Windows via the standard Go cross-compilation flags (`GOOS`/`GOARCH`).

The build command below writes `bin/sentinel.exe`. The `.exe` name reflects a Windows development host — it is just an output filename, not a platform constraint. On Linux or macOS, name the output whatever suits your OS (for example, `bin/sentinel`).
...
The reference runs three localhost listeners. Two are Sentinel's own; the third is your origin application, which Sentinel fronts.
...
| `:8445` | Separate authenticated admin listener (Audit Console + admin API), cross-origin to the edge | `-admin-addr ":8445"` |
| `:9000` | Your origin app that Sentinel proxies to (the `-upstream` target) | `-upstream "http://127.0.0.1:9000"` |

The admin listener is deliberately separate from the public edge: `/__hmn/admin/*` is not reachable on the edge and returns `404` there. The Audit Console is served at `https://localhost:8445/__hmn/admin/console`. For the full flag list and defaults, see [CLI, config & policy](cli-config-policy.md).
...
In the reference, there are **none**. Sentinel holds its state in memory in a single process:
...
Once the binary builds, start with monitor mode so Sentinel scores and logs traffic without enforcing any verdict. Follow the step-by-step [monitor-mode quickstart](../tutorials/quickstart-monitor-mode.md), then read the [integrator start-here guide](../start-here/integrator.md) for how to place Sentinel in front of your origin.
...
- [Integrator start-here](../start-here/integrator.md) — putting Sentinel in front of your app.
- [How Sentinel sees a request](../concepts/how-sentinel-sees-a-request.md) — the L1–L7 detection model.
