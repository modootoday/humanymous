---
description: "humanymous Gate install requirements: Go 1.25.3, four direct dependencies (uTLS plus golang.org/x/crypto, net, text), pure Go with no CGO, cross-compiles to Linux, macOS, Windows. Reference build needs no database or Redis."
...
The detection engine is published too, as `ghcr.io/modootoday/humanymous-core:latest` (for standalone self-testing / the demo). See [Run the detection engine](../how-to/run-detection-engine.md) for the full Core runbook (flags, ports, and the published-image `docker run` recipes).
