...
| Network residual policy | Enforced policy selects a residual class of transport evidence: proxy hops, virtual private networks, Tor, or source spoofing; a Transport-Layer-Security handshake that omits an extension every current browser sends (the post-quantum key share, application settings, certificate compression, or encrypted client hello); a Hypertext-Transfer-Protocol-version-2 profile inconsistent with the claimed browser (settings, connection flow-control window, stream priority, or frame order); or a browser Transport-Layer-Security stack delivered over Hypertext Transfer Protocol version 1.1 | CHALLENGE | Operator-controlled and heuristic; each residual class is individually settable to monitor for a network with a legitimate middlebox (an inspecting proxy, a policy that strips an extension, or a downgrade), so a deployment can quiet one class without disabling the rest; Transmission Control Protocol residuals never auto-enforce |
...
| Transport-Layer-Security handshake missing a current-browser extension | Network and protocol inspection |
| Hypertext Transfer Protocol version 2 profile inconsistent with the browser claim | Network and protocol inspection |
| Browser Transport-Layer-Security stack delivered over an older Hypertext Transfer Protocol version | Cross-layer consistency |
| Request header order that does not match a real browser | Network and protocol inspection |
