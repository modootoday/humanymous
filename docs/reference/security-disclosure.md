...
**Audience:** security researchers reporting a vulnerability in humanymous Gate, and evaluators performing security due diligence.
...
- The detection engine (the seven-stage pipeline), the verdict pipeline (ALLOW / CHALLENGE / DENY) and enforcement rules, where a flaw crosses a documented security boundary — for example a control-plane injection, an authentication or role-based access control bypass, an audit-log integrity break, or a verdict-token forgery not already described as an accepted residual.
...
> **Note:** A verdict of ALLOW, CHALLENGE or DENY that you disagree with is **not** by itself a vulnerability. Detection is heuristic and bounded: client and network signals alone cannot reliably separate coherent browser or human-assisted automation from legitimate traffic, and safe methods on non-strict routes have a documented fail-open residual. These are stated design limits, not defects. See [enforcement rules, verdicts and signal reference](hard-rules-verdicts.md). If you believe you can bypass an **enforcement rule** documented as high-confidence, that is in scope — report it.
...
- Detection outcomes that reflect documented limits: the coherent-automation detection boundary (internally consistent browser automation or human-assisted automation), the accepted fail-open residual for safe Hypertext Transfer Protocol methods on non-strict routes, and the tamper-**evident** (not tamper-proof) in-window residual for audit records written after the last signed checkpoint.
