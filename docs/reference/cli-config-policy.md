...
| `-audit-wal` | `""` (ephemeral) | Durable audit WAL directory (survives restarts). With `-audit-verify`, **required** and must be non-empty: replays the chain, checks HMACs/STHs/**witness** co-signatures, exits non-zero on empty chain or any break. |
