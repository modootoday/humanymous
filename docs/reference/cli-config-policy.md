...
keywords: ["humanymous Gate CLI flags","per-route policy presets","reverse-proxy config reference","monitor mode","strict and attested routes","ALLOW CHALLENGE DENY thresholds","admin API RBAC","rate limiting and bans","route table longest-prefix","self-hosted bot detection config"]
---
...
| `-audit-wal` | `""` (ephemeral) | Durable audit WAL directory (survives restarts). With `-audit-verify`, **required** and must be non-empty: replays the chain, checks HMACs (when a keystore supplies the key)/STHs/**witness** co-signatures, exits non-zero on empty chain or any break. Without `-keystore`, `-audit-verify` does **not** mint a random HMAC (that would always fail `hmac-invalid`); HMAC is skipped (`hmac-unchecked` path in the library). For a full verify of a production WAL, pass the matching `-keystore` + `HMN_UNSEAL`. Boot also logs `audit keys: … sth_public=… witness_public=…` (public material only). |
...
| `-trusted-proxies` | `""` (disabled) | Comma-separated CIDRs of L4 balancers allowed to send a PROXY-protocol-v2 header. **Caveat:** set this to your balancers' addresses ONLY — never a broad range like `0.0.0.0/0`, or any client could spoof its source IP. The Gate still terminates TLS (JA3/JA4 unaffected). |
...
The kill switch is a fleet-wide dual-control flip that demotes hard-rule enforcement to monitor across all nodes (detection stops, traffic flows). Manual bans still enforce under the kill switch.

> **Warning:** The kill switch is fleet-wide. Flipping it stops verdict enforcement on every node at once. It is a dual-control action (see [Admin API](#admin-api-admin-listener)).
...
The client-facing control plane is served on the public edge listener under the prefix `/__hmn/` (`Config.ControlPath = "/__hmn/"`). The injected detection bundle loads via the control plane, and the browser beacons signals to `/__hmn/collect` on the same TLS connection the server uses to capture L5 (TLS/HTTP2) signals.
...
| *(challenge / PoW interstitial)* | Served from the control plane. |
...
| `/__hmn/admin/erasure` | Operator or DPO | Yes — committer must be a distinct DPO | Starts a crypto-shred with a hold window. Requesting needs Operator or DPO; committing requires the DPO role. |
| `/__hmn/admin/killswitch` | Operator | Yes — distinct Approver | Fleet-wide demotion of hard-rule enforcement to monitor. |
| `/__hmn/admin/erasures/{id}/cancel` | Operator / DPO | — | Cancels an erasure during its hold window. |
| `/__hmn/admin/approvals/{id}` | Approver, or DPO for erasure (≠ requester) | Commits a pending action | A distinct approver commits a pending dual-control action. Ban / kill-switch approvals need the Approver role; **erasure approvals require the DPO role** — a generic Approver cannot commit an erasure. |

> **Note:** Dual-control means a distinct second identity commits the action: an Approver for permanent/CIDR bans and the kill switch, and a distinct DPO for erasure. The requester cannot approve their own action.
