...
2. **Read the cost off the audit stream, KPIs, and the built-in probes.** The reference observability surface is the audit stream (`GET /__hmn/admin/audit` with filters and a cursor), the Integrity view/endpoint, the Overview KPIs in the Ledger, a Prometheus text-exposition gauge snapshot at `GET /__hmn/admin/metrics` on the admin plane, and `GET /__hmn/healthz` (liveness) / `GET /__hmn/readyz` (readiness — 503 while draining) on the edge. SIEM shipping of the audit stream is still a prod-delta. See [Observability & SIEM](../how-to/observability-siem.md).
