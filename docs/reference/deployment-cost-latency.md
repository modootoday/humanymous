...
humanymous Gate ("Gate" after first mention) is the reverse-proxy enforcement layer. It injects its current collector where configured, scores the evidence available at Gate, enforces the verdict, and records decisions. Core's complete seven-stage evidence path is a separate benchmark surface.

For the reference-vs-production boundary, see [Production-ready vs reference (production responsibility)](./production-vs-reference.md). To capture your own numbers, see [Observability & SIEM](../how-to/observability-siem.md) and [Self-validation & red-team](../how-to/self-validation-red-team.md). For the request-path impact on your app's behavior, see [Will this break my app?](../explanation/will-this-break-my-app.md).
...
- **TLS termination.** Gate terminates TLS at the edge (minimum TLS 1.2). That is the standard handshake-plus-symmetric-crypto cost of any terminating proxy, and it is also what makes the network and protocol signals (JA3/JA4-family TLS fingerprint, HTTP/2 fingerprint, header order) observable at all.
...
- **Inline scoring.** Gate aggregates the evidence its current collector supplies into a 0–100 risk score and applies the shared enforcement-rule table before the verdict takes effect. Core's full seven-stage evidence set is not available at the current Gate, so benchmark the component and topology you will deploy.
...
- **The kill switch is node-local.** It demotes rule enforcement for every route on the current node, while detection continues and manual bans still enforce. Fleet coordination is a production responsibility.

> **Warning:** The kill switch is **node-local but not route-scoped**. Rehearse the dual-control path and your external multi-node coordination before you need it.
...
- **There is no shared/HA state in the reference.** A shared store (for example Redis) for fleet-consistent verdicts and bans, high availability, and horizontal scale is **production responsibility** — a production responsibility, not something the reference binary provides.
...
See [Production-ready vs reference (production responsibility)](./production-vs-reference.md) for the full list of what a production deployment must add, including the shared fleet-state layer.
...
2. **Read the cost off the audit stream, KPIs, and the built-in probes.** The reference observability surface is the audit stream (`GET /__hmn/admin/audit` with filters and a cursor), the Integrity view/endpoint, the Overview KPIs in the Ledger, a Prometheus text-exposition gauge snapshot at `GET /__hmn/admin/metrics` on the admin plane, and `GET /__hmn/healthz` (liveness) / `GET /__hmn/readyz` (readiness — 503 while draining) on the edge. SIEM shipping of the audit stream is still a production responsibility. See [Observability & SIEM](../how-to/observability-siem.md).
...
- [Production-ready vs reference (production responsibility)](./production-vs-reference.md) — the full reference-vs-production boundary, including shared fleet state and HA.
