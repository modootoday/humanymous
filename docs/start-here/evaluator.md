---
description: "Reverse-proxy bot detection you can evaluate: it scores each request across seven layers into an ALLOW / CHALLENGE / DENY verdict. Raises the cost of automation; documents its coherent browser or human-assisted automation ceiling. Run monitor mode first."
keywords: ["evaluate open source bot detection","self-hosted bot detection","reverse proxy bot detection","ALLOW CHALLENGE DENY verdict","monitor mode evaluation","seven-layer detection","coherent browser or human-assisted automation ceiling","self-hosted DataDome alternative","humanymous Gate","graded risk score"]
---
...
<picture>
  ...
  <img src="../assets/screenshots/framed/verdict-detail-light.webp" alt="A humanymous /demo verdict card showing a DENY decision with its risk score and contributing enforcement rules" />
</picture>
...
1. **[What Gate Is (and Is Not)](../explanation/what-gate-is.md)** — The scope, the design principles (defense-in-depth, cross-check-first, low false-positive, deterministic), and the five attacker cost bands, from direct HTTP automation through coherent browser or human-assisted automation attacker tiers, including where the coherent browser or human-assisted automation ceiling sits. Start here to calibrate expectations before you look at mechanics.
...
> **Read before you benchmark:** [Supported topologies](../reference/supported-topologies.md) explains why the numbers you measure depend on *where you place it*. The network plane (JA3/JA4/H2) only fires when the process terminates raw TLS itself — it is inert behind a CDN/application-layer load balancer and absent at the `cmd/gate` proxy — so benchmarking the Core engine and deploying the gate, or sitting behind a CDN, measures a different detector than you deploy.
