---
description: "Developer hub to extend the open-source bot detection engine: add signals, scoring, and enforcement rules, then run the local Red test catalog against your own 127.0.0.1."
...
3. [Detection engine internals](../explanation/detection-engine-internals.md) — signals, the scoring math (dedup → per-stage cap → noisy-OR), the enforcement-rule table, and the ScoreTrace you can observe.
4. [Extend detection](../how-to/extend-detection.md) — add a signal or an enforcement rule and confirm it in the trace.
...
4. [Red-team catalog](../reference/red-team-catalog.md) — the profiles, their labels, and the enforcement rule each is expected to exercise.
