...
**Audience:** security-operations and incident responders operating a running humanymous Gate deployment.

If you have never seen the Ledger, read [How Gate sees a request](../concepts/how-gate-sees-a-request.md) first.
...
flowchart TD
  ...
  FP --> KS["Broad lockout only: kill switch — node-local, dual-control"]
...
humanymous Gate is a reverse-proxy enforcement layer that scores the evidence its current collector supplies, applies enforcement rules, and enforces an ALLOW, CHALLENGE, or DENY verdict at the edge. The standalone Core engine demonstrates a richer seven-stage evidence path; do not use Core measurements as Gate incident baselines.
...
1. **Kill switch** (node-local).
2. **Permanent or CIDR ban placement**.
3. **Erasure** (data protection officer-gated commit — not a generic Approver).
...
2. **Read** the TopContributors on the drilled-down sessions. Real automation clusters on high-confidence signals (`l1.navigator.webdriver`, `l1.cdp.proxy_leak`, `x.ua_vs_ja4`). A false-positive storm clusters on a single route or a single heuristic rule (for example no interaction observed rule, no-interaction → CHALLENGE) firing on ordinary traffic.
...
- **Misfire is broad / cross-route and users are locked out node-local** → consider the **kill switch**. Node-local and dual-control; read the warning first.
...
> **Warning:** The kill switch is **node-local** and **dual-control**. It demotes rule enforcement for every route on the current node. Detection continues, manual bans still enforce, and a multi-node deployment must coordinate other nodes separately.
...
- **Consequence:** rule enforcement is off node-local. The origin is exposed to automation that enforcement rules would otherwise DENY.
...
- The auto-ban ladder is climbing; cross-session correlation or decision-probing sweep protection appears in the audit stream.
...
- **One fingerprint, many subnets** → ban on the **`fp:` key, not `ip:`**. An IP ban chases a moving target; the fingerprint is the stable handle.
...
## (c) HTTP/2 flood / destructive abuse (Hypertext Transfer Protocol version 2 denial-of-service protection rule)

Protocol-level flooding or a destructive request pattern.

### Trigger signal

- Hypertext Transfer Protocol version 2 denial-of-service protection matches.
...
2. **Confirm** it is protocol abuse, not a load spike: this rule keys only on Rapid Reset or continuation-frame flooding. The application request-rate limiter and ban ladder are separate controls.

### Decision tree

- **The protocol denial-of-service rule matched** → DENY is already active and the origin is not contacted for the scored request. A simultaneous `429` means the separate rate limiter is also shedding traffic.
...
- **Rate ceilings are too low and catching real bursts** → use Settings to dry-run a rate-window or threshold change. The server classifies a reduction in protection and requires approval when appropriate; approved values hot-apply to new requests.
...
- **Do not** reach for the kill switch here — it demotes rule enforcement on the current node and would remove the protocol denial-of-service DENY that is protecting the origin.
...
## (d) AI-agent wave (automated-browser interaction signature rule)

A surge of AI browser-agents driving real browser engines.

### Trigger signal

- The automated-browser interaction signature matches across many sessions.
...
2. **Inspect** the behavioral evidence. The rule requires a click without an approach trajectory plus a second machine-interaction or browser-control indicator. These are behavioral tells on real engines, so read representative drill-downs before acting broadly.
...
- **Clear agent signature, high volume** → the rule is already denying. Add fingerprint bans for persistent sources to escalate the ladder.
...
> **Note:** coherent browser or human-assisted automation (anti-detect tooling plus real-human click-farms) is an explicit design boundary Gate does not solve; it is mitigated only by rate and reputation. If bans are not denting the volume, escalate rather than banning ever-wider.
...
> **Note:** verification is a mode of the `gate` binary, not a separate tool. Run `bin/gate.exe -audit-wal <wal-dir> -keystore <keystore-path> -audit-verify` with `HMN_UNSEAL` set. **`-audit-wal` is required** and the write-ahead log must contain at least one record — an empty chain exits non-zero (`class=empty-chain`) so a missing path cannot look like a clean audit. On a real chain it re-checks the hash chain, per-record HMACs, Ed25519 Signed Tree Heads, and independent-witness co-signatures, then prints `audit-verify: OK (<n> records)` and exits 0 on success, or `audit-verify: FAILED class=<class> seq=<n>` and exits non-zero on a break. Point the keystore + `HMN_UNSEAL` at the node under investigation; there is no separate offline-verifier executable.
...
- **Page the data protection officer** as well if the affected segment touches subject data or an in-flight erasure — see the [Right-to-erasure (crypto-shred) runbook](./erasure-crypto-shred.md).
...
The audit chain and its pseudonyms are only *pseudonymous* — Gate has no re-identification API; anyone holding the **keystore + `HMN_UNSEAL`** can resolve offline alone (custodial, not dual-control). Events that raise a session/pseudonym incident to a **personal-data breach assessment** are those that compromise that custody or the chain itself:
...
2. **Page the data protection officer / privacy owner immediately** — in parallel with the security lead, not after. The breach assessment (scope, affected subjects, likelihood of harm) is theirs to run.
3. **Start the clock and record it.** Log the **time of awareness** — under GDPR the controller's assessment feeds a **72-hour notification window to the supervisory authority** (Art. 33), with data-subject notification (Art. 34) when the risk warrants. This runbook does not decide whether the threshold is met and does not draft the notice — **the controller / data protection officer owns the legal determination and wording**. Your job is to surface the facts (what was exposed, when, how many subjects, mass-re-identification feasible yes/no) so they can decide against the clock.
4. **Scope the blast radius with the data protection officer:** how many un-shredded subjects were resolvable, whether the vault/keystore was actually used to re-identify, and whether the exposure is contained.
...
- **Page the security lead** (technical containment) **and the data protection officer / privacy owner** (breach assessment + the notification clock) **together**.
- Do not wait for the technical root-cause to be fully understood before paging the data protection officer — the 72-hour clock runs from awareness, not from resolution.
...
| --- | --- | --- | --- |
| (a) DENY spike, humans locked out | no interaction observed rule misfire / route-scoped | Demote route to monitor | Distinct Approver for kill switch |
| (b) Scraping / cred-stuffing | cross-session correlation rule, decision-probing sweep protection | Temporary `fp:` ban | Distinct Approver for permanent/CIDR |
| (c) HTTP/2 flood | Hypertext Transfer Protocol version 2 denial-of-service protection rule, `l5.h2dos.rapid_reset` | Confirm auto-DENY + limiter | Infra/network on-call |
| (d) AI-agent wave | automated-browser interaction signature rule, `l4.agent.burst_silence` | Temporary `fp:` ban | Distinct Approver; Operator for route review |
| (e) Integrity red | mismatch classes | Run offline verifier, preserve | Security lead (+ data protection officer if data-touching) |
...
| (g) Personal-data-breach tie-in | keystore/`HMN_UNSEAL`/vault compromise; `audit.integrity.violation.detected` | Preserve evidence, log time of awareness | Security lead **and** data protection officer together (72h clock) |
...
- **The reference kill switch is node-local** — a multi-node deployment needs external coordination before this action is node-local. Manual bans still enforce; restore rule enforcement through dual control when the cause is fixed.
...
- [enforcement rules, verdicts & signal reference](../reference/hard-rules-verdicts.md)
