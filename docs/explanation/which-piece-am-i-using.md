...
The engine's core flags for local runs are `-addr`, `-web`, and `-rit` (default `true`); it also accepts TLS controls, shared operational-log controls, and `-ops-token` (see [run the detection engine](../how-to/run-detection-engine.md) for the full Core flag set). The Observatory appears on that same server only when it is dev-gated on with `HMN_PLAYGROUND=1`, and it refuses to start on a non-loopback `-addr` (fail-closed).
