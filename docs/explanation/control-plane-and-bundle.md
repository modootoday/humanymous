...
keywords: ["/__hmn/ control plane","injected detection bundle","loader.js","csp-report endpoint","CSP conflict debugging","SPA router collision","admin plane isolation","humanymous Gate reverse proxy","streaming HTML injection","Gate collection boundary"]
---
...
- **`/__hmn/collect`** — the signal beacon. The browser posts the client evidence collected by Gate's current loader. Core can correlate a richer browser report with directly captured connection evidence; the current Gate does not extract that full ClientHello and HTTP/2 set.
...
When a verdict is CHALLENGE, Gate returns HTTP 401 with `Cache-Control: no-store` and does not contact the origin. The reference response says that another check is required and loads `/__hmn/loader.js`, but it does **not** provide a complete usable recovery path for every visitor. A deployment must wire and test its own self-hosted, accessible verification flow before telling users that waiting, retrying, proof of work, or Pass will let them continue. See [Why am I seeing this?](../help/why-am-i-seeing-this.md) and [Challenge accessibility](../help/challenge-accessibility.md) for honest end-user wording.
...
| `/__hmn/collect` | POST | Browser signal beacon for Gate's current collector | **200**, JSON `{"sessionId","riskScore","verdict","hardRuleFired"}`; non-POST returns **405** | No — served by Gate |
| `/__hmn/csp-report` | POST | CSP violation report sink | **204** No Content | No — served by Gate |
| Challenge response | (served on CHALLENGE) | Minimal reference response; production must provide and test a usable verification flow | **401**, `Cache-Control: no-store` | No — origin never contacted |
...
For the complete Core model and the smaller evidence set currently available to Gate, see [How Gate sees a request](../concepts/how-gate-sees-a-request.md).
...
- [How Gate sees a request](../concepts/how-gate-sees-a-request.md) — the Core pipeline, Gate boundary, and shared terminology.
