...
humanymous Gate ("Gate") is a reverse-proxy enforcement layer that scores the evidence its current collector supplies, enforces ALLOW, CHALLENGE, or DENY at the edge, and writes decisions to a tamper-evident audit log. Core's richer seven-stage collection path is a separate processing surface. This document does not repeat the field-level processing register; read it alongside the [data-processing inventory](../reference/data-processing-inventory.md), the [standards mapping](../reference/standards-mapping.md), the [erasure runbook](../runbooks/erasure-crypto-shred.md), and [role-based access control and separation of duties](../reference/rbac-separation-of-duties.md).

> **Important:** This document describes a **reference implementation**, not a production-hardened product. Several controls a DPIA would rely on in production are production responsibilities (identified inline). Gate is processing **tooling**; determining lawful basis, necessity, proportionality, and the DPIA conclusion for a specific deployment remains the responsibility of the operator acting as **data controller**.
...
Necessity turns on whether the purpose can be met with less personal-data processing. Document only the evidence the deployed component actually collects. Core's full model includes browser, interaction, and network-protocol evidence; the current Gate supplies a smaller set. A cross-check-first design avoids treating one identifier as decisive, but it does not justify claiming collection that is absent from the deployed path.
...
Proportionality must account for the impact on **legitimate human users** who are wrongly challenged. Several rules use high-confidence automation evidence, while heuristics can match some humans and therefore issue CHALLENGE rather than DENY. CHALLENGE is still real user impact, and the reference Gate does not provide a complete recovery flow for every visitor. The controller must document challenge friction, effective lockout risk, and the accessibility of the deployed flow as part of the balancing test. See also [Will this break my app?](will-this-break-my-app.md).

> **Note:** The reference challenge is a minimal response, not a complete visitor-recovery experience. The accessibility of the flow actually deployed is a proportionality input the impact assessment should record.
...
- **Behavioral timing is biometric-adjacent and component-scoped.** Core's interaction-analysis stage evaluates mouse-movement and keystroke *timing dynamics* (dwell/flight, velocity, trajectory) when the **full client detection path** (Core and its WebAssembly bundle) is in use. Depending on jurisdiction and how it is used, timing-dynamics profiling can attract heightened scrutiny under the General Data Protection Regulation or the Personal Information Protection Act. Controllers who deploy **only the Gate reverse proxy** must **remove "interaction timings" / mouse-keystroke dynamics from their Article 13 notice** unless they also run a client path that actually collects them — Gate's injected loader does not reproduce the Core behavioral sample stream. Where the full path is enabled, the reference collects timing features as an input to the security verdict; it is not used to uniquely identify a person and is stored only as aggregate features under audit pseudonymization. There is currently **no built-in toggle** to disable behavioral collection — consent-gated collection is a production responsibility.
- **GPC / DNT are read as a *privacy-posture* signal, not honored as a tracking opt-out.** A session presenting Global Privacy Control or Do-Not-Track (or Brave shields, ad-block, tracking protection) is treated as a **human/false positive-mitigation** signal — it *lowers* suspicion and damps fingerprint-noise scoring — but it does **not** exempt the session from any server-authoritative enforcement rule: cross-session correlation rule (residential-proxy rotation) is deliberately never gated on a client-reported privacy flag, precisely so a proxy scraper cannot post `adBlock:true` to disarm it. It is **not** honored as an opt-out of the detection processing itself, because bot detection is security processing (typically legitimate interests / strictly-necessary), not advertising/tracking to which GPC/DNT legally attach. If your lawful-basis analysis concludes the processing is not strictly necessary for your context, you must add your own consent/opt-out gate; the reference does not do this for you.
- **The optional behavioral model is audit-only and human-supervised.** If the Core loads the optional behavioral model, its output is a **weight-0 / audit-only** signal that does not affect any verdict, so it makes **no automated decision about a person** (relevant to the Article 22 posture the transparency report states). Its threshold self-calibrates from an **independent oracle** — a solved challenge is a confirmed human, and a *failed* one is never labelled a bot (protecting the very users the false-positive path exists for) — never from the model's own output; and an autonomous canary can only **disable** the model (reverting to heuristics), never promote it. Its accessibility cohort exists so the *offline* retrain gate enforces false-positive ≈ 0 **per input modality**, so a regression against keyboard-only / switch-access users cannot hide behind the pointer-using majority. If you enable its **lab-only training-trace store**, confirmed-human behavioral aggregates are written to disk for retraining — a distinct purpose, outside the crypto-shred erasure boundary, that you must inventory (see [Data processing & personal-data inventory](../reference/data-processing-inventory.md)). Letting this residual ever affect a verdict is a deliberate, separately-approved change, not a default.
...
- **What dual-control actually covers.** Product dual-control (distinct second role) applies to **erasure**, **permanent/CIDR bans**, and the **kill switch** — not to re-identification. See [role-based access control and separation of duties](../reference/rbac-separation-of-duties.md).
...
> **Note:** A standalone offline verifier **process**, RFC 3161 trusted timestamps, and external WORM/object-lock anchoring are production responsibilities (Section 7.3). They are recommended production anchors, not shipped features of the reference.
...
- **role-based access control with four roles** — Auditor, Operator, Approver, data protection officer — with capabilities scoped per role (see [role-based access control and separation of duties](../reference/rbac-separation-of-duties.md)).
- **Separation of duties.** Read, operate, approve, and data protection officer-erasure capabilities are held by distinct roles; no single role holds all.
- **Dual-control** on the highest-impact actions: permanent/CIDR bans and the kill switch must be committed by a **distinct Approver**; **erasure** must be committed by a **distinct data protection officer** (a generic Approver cannot approve erasure).
...
> **Warning:** The **kill switch is node-local**. Activating it demotes rule enforcement to monitor mode across the entire fleet — detection continues and affected traffic flows to the origin — while manual bans continue to enforce. Its scope and dual-control (a distinct Approver) must be understood before use; record it in the DPIA as a control with node-local blast radius.

> **Note:** In the reference, admin authentication is bearer tokens with dev defaults; real KMS/HSM key custody, ACME certificates, and mutual Transport Layer Security/SSO admin authentication are production responsibilities. The DPIA should record the production key-custody and admin-auth model the operator will actually deploy.
...
The reference **declares** HOT / WARM / COLD age labels for documentation and a `Tier(age)` helper used in tests. **They are not enforced at runtime:** no production caller prunes, compresses, or relocates the write-ahead log by tier; the Ledger Retention card is **hardcoded HTML**, not a read of `/admin/policy`. There is **no WORM** media or object-lock integration in the reference.
...
**Audit durability defaults (honest residual):** without `-audit-wal` the chain is **in RAM and lost on restart**; with `-audit-wal` it **grows without bound**. Neither default implements a retention schedule. **Mitigation in the reference: none** — operators must supply external retention, archival, and deletion policy (production responsibility).
...
```mermaid
flowchart LR
  W["Decision written · (pseudonymized, hash-chained)"] --> A["Write-ahead log or in-memory chain · (no automatic tiering)"]
  A -. "operator retention policy · (production responsibility)" .-> RET["Archived / retired"]
  ER["Cryptographic erasure · (data protection officer + dual-control, about 5-minute hold)"] -. "destroy per-subject linkage key" .-> UN["Record retained, chain intact · no longer linkable to subject"]
  A -. any age .-> ER
```
...
- **data protection officer gating and dual-control.** Erasure is committed by a distinct data protection officer; a generic Approver cannot.
...
- **Completion evidence** sealed on **execution** — when the hold window elapses and the shred runs, not at commit; a cancelled erasure produces none. Evidence is an `erasure.completed` audit record (HMAC-chained; covered by the next STH), not a standalone independently-signed certificate blob (retrieval endpoint remains production responsibility).
...
### 7.3 Physical retirement and external anchoring are production responsibilities
...
- **Physical WORM retirement at COLD expiry** — the physical destruction or retirement of records once the COLD tier expires — is a production responsibility.
- **External WORM/object-lock anchoring and RFC 3161 trusted timestamps** — anchoring the chain to an external immutable store or a trusted timestamp authority — are production responsibilities. They are recommended production anchors that would further constrain the in-window residual described in Section 5, but the reference does not ship them.
...
RFC 3161 trusted timestamps and S3 Object-Lock/WORM external anchoring are **production responsibilities** — present them as recommended production anchors, not as controls the reference provides.
...
| --- | --- | --- | --- |
| R1 | Human users wrongly challenged (false positive) | Low-false positive design; heuristic rules challenge rather than block; monitor mode to measure before enforcing | Document false positive residual and challenge accessibility (challenge WCAG UI is production responsibility) |
...
| R5 | In-window history rewrite before next checkpoint | Witness co-sign stops silent rewrites; checkpoint every 32 records; public verification | Accept documented in-window residual; consider external anchoring (production responsibility) |
| R6 | Privileged admin misuse | role-based access control, separation of duties, dual-control, deny-by-default, server-derived actor, meta-audit | Deploy production admin auth (mutual Transport Layer Security/SSO is production responsibility) |
| R7 | Over-retention | **None in the reference** (labels only; write-ahead log unbounded or RAM) | Operator retention schedule / archival / delete policy (production responsibility) |
| R8 | Erasure breaking integrity / WORM conflict | Crypto-shred (key destruction), records retained, chain intact | Confirm subject and hold window; erasure is data protection officer dual-control |
| R9 | Behavioral training-trace store: confirmed-human aggregates written to disk un-pseudonymized, un-chained, for a distinct (retraining) purpose; each record carries an input-modality **cohort** (`keyboard` correlates with assistive-technology use) | Off by default (lab/operator opt-in); solved-challenge humans only; timings-only aggregates (no key values); `0600` file / `0700` dir; cohort is a coarse inference, not a declared disability | **Outside crypto-shred** (no subject key → per-subject erasure cannot reach it): add a file-level retire policy to your Article 17 procedure; inventory the store, its purpose, and its retention. **Whether the AT-adjacent `keyboard` cohort is Article 9 special-category data is a controller determination** — record it. |
| R10 | Fleet correlation coordinator (`-redis`): device-fingerprint → {`/24` locations, timing} graph written to Redis at rest (values raw, key HMAC/SHA-256-digested) | ~60-min key TTL (self-clearing); weight-0 audit signal only (no verdict); `HMN_REDIS_KEY` HMAC-binds the key | **Outside crypto-shred** (digest ≠ subject key); disclose as a coordinator-side store in the RoPA; set `HMN_REDIS_KEY` (else the key digest is a reversible fingerprint-confirmation oracle) + AUTH + network-isolate the coordinator |
...
- [role-based access control and separation of duties](../reference/rbac-separation-of-duties.md) — roles, capabilities, and dual-control model.
