...
| R9 | Behavioral training-trace store: confirmed-human aggregates written to disk un-pseudonymized, un-chained, for a distinct (retraining) purpose | Off by default (lab/operator opt-in); solved-challenge humans only; timings-only aggregates (no key values); `0600` file | **Outside crypto-shred** (no subject key → per-subject erasure cannot reach it): add a file-level retire policy to your Article 17 procedure; inventory the store and its purpose; operator-defined retention (no TTL in the reference) |
