...
- Open the Integrity / Chain Integrity view (or `GET /__hmn/admin/integrity`) and re-verify the chain. Expect `ok:true` with records present and no `checkpoint-mismatch` / `node-missing` / `empty-chain` classes introduced by the restart.
...
> **Note:** the reference emits an `instance.startup` audit record on every boot (the first record written after key load), so each restart lays down a fresh start marker that brackets the "after upgrade" records — combine it with the `config_version` comparison above. There is no matching stop marker: an `instance.shutdown` event type is defined but the reference never emits it, so bracket on the post-restart `instance.startup`, not on a clean-stop record.
