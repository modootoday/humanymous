...
**Audience:** defensive-test and detection developers who want to build and run the standalone engine (`bin/server.exe` on `127.0.0.1:8443`) directly.
...
The three flags above cover most local runs; the remaining flags configure TLS, logging, and the operator surface.
...
| `-log-console-format` | `plain` | Console format: `off\|plain\|jsonl`. Also settable via `HMN_LOG_CONSOLE_FORMAT`. |
...
rotated logging volume and never on an audit, key, or settings volume.
