...
The Integrity view (also labelled "Chain Integrity") verifies the audit log **live**. The log is an append-only hash chain with a per-record HMAC and Ed25519 Signed Tree Heads, and the important property is on the page: it **verifies with the public key alone**. You do not need any secret to confirm the log has not been rewritten.
...
This view covers cryptographic erasure (crypto-shred) and retention. A right-to-erasure request is honored by **destroying the per-subject linkage key** — the chain and its Merkle anchors stay intact and verifiable; the records are never deleted, but the subject can no longer be re-linked to them. Retention runs in tiers (HOT ~90 days / WARM ~1 year / COLD ~7 years).
