...
<video autoplay loop muted playsinline poster="../assets/screenshots/anim/quickstart-cast-poster.webp" width="820" style="max-width:100%;border-radius:12px" aria-label="A self-validation run: `docker compose run --rm bots` streams the automation catalog against the local engine — each simulator resolves to DENY with its hard rule, the honest-human baseline clears ALLOW, ending 45/45 blocked with 0 false positives.">
