...
keywords: ["self-hosted bot detection validation","red-team your own deployment","measure false-positive rate","bot detection true-positive rate","botTPR humanFPR","monitor mode before enforce","Selenium Puppeteer Playwright simulators","honest-human baseline","humanymous Gate","coherent browser or human-assisted automation ceiling"]
---
...
<video autoplay loop muted playsinline poster="../assets/screenshots/anim/quickstart-cast-poster.webp" width="820" style="max-width:100%;border-radius:12px" aria-label="A self-validation run: `docker compose run --rm bots` streams the automation catalog against the local engine — each simulator resolves to DENY with its enforcement rule, the honest-human baseline clears ALLOW, ending 45/45 blocked with 0 false positives.">
