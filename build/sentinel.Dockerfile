# humanymous Sentinel — the reverse-proxy security layer (SoT-19..28).
...
# the adjacent sentinel.Dockerfile.dockerignore re-includes cmd/sentinel.
...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -trimpath -ldflags="-s -w" -o /out/sentinel ./cmd/sentinel/

FROM gcr.io/distroless/static-debian12:nonroot
WORKDIR /app
COPY --from=builder /out/sentinel /app/sentinel
EXPOSE 8444 8445
USER 65532:65532
