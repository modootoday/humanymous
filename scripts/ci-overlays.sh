...
BASE_PROJECT="${E2E_BASE_PROJECT:-deployments}"
OVERLAY_TIMEOUT="${E2E_OVERLAY_TIMEOUT:-240}"
OVERLAY_HEARTBEAT_SECONDS="${E2E_HEARTBEAT_SECONDS:-30}"
cd "$(dirname "$0")/.."
...
run_timed() { # label command...
  ...
  command_pid=$!
  (
    while sleep "$OVERLAY_HEARTBEAT_SECONDS"; do
      kill -0 "$command_pid" 2>/dev/null || exit 0
      echo "[overlay] HEARTBEAT phase=$label pid=$command_pid"
    done
  ) &
  heartbeat_pid=$!
...
run() { # name  "services"  assertion-service  [wait]
  ...
  for attempt in 1 2 3; do
    echo "[overlay] START name=$name attempt=$attempt timeout=${OVERLAY_TIMEOUT}s"
    if ! run_timed "$name/up" docker compose $BASE -f "deployments/compose/$name.yaml" up -d $services; then
      echo "[overlay] FAIL name=$name phase=up attempt=$attempt"
      docker compose $BASE -f "deployments/compose/$name.yaml" ps || true
      docker compose $BASE -f "deployments/compose/$name.yaml" down -v >/dev/null 2>&1 || true
      continue
    fi
    docker compose $BASE -f "deployments/compose/$name.yaml" ps
    sleep "$wait"
    if run_timed "$name/assert" docker compose $BASE -f "deployments/compose/$name.yaml" run --rm -T "$assertion"; then
      echo "[overlay] PASS name=$name attempt=$attempt"
      ok=1
      break
    fi
